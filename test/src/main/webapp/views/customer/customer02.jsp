...
<body>
      ...
      <table>
        ...
        <tbody>
          <c:forEach var="qna" items="${qnalist}">
            <tr>
              <td><a href="<c:url value="/customer/customer02_3"/>?idx=${qna.qnaIdx}">${qna.qnaIdx}</a></td>
              <td>${qna.qnaTitle}</td>
              <td>${qna.qnaName}</td>
              <td>${qna.qnaDate}</td>
            </tr>
  ...
  <div class="search container">
    <div>
      <img onclick="window.location.href='<c:url value="/customer/customer02_2" />'"
      src="<c:url value="/img/customer/write.gif" />">
    </div>
    <form action="<c:url value="/customer/customerOneSearch"/>" method="GET">
      <table>
        <tr>
          <td>
  ...
  <script>
    function openWindow( no ){
      window.open('customer02_3.jsp?no='+no, '비밀번호확인', 'width=430,height=300,location=no,status=no,scrollbars=no')
    }
  </script>
</body>
</html>